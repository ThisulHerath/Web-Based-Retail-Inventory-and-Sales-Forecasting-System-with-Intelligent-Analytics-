...
export const getAllPurchases = async (req, res) => {
    try {
        ...
        if (parsedStartDate || parsedEndDate) {
            query.purchaseDate = {};
            if (parsedStartDate) query.purchaseDate.$gte = parsedStartDate;
            if (parsedEndDate) query.purchaseDate.$lte = parsedEndDate;
        }
...
export const createPurchase = async (req, res) => {
    try {
        ...
        for (const item of lineItems) {
            ...
            if (!inv) inv = await Inventory.create({ product: item.product, displayedStock: 0, storedStock: 0 });
            // Supplier purchases go to stored stock (not displayed/shelf stock).
            inv.storedStock += item.quantity;
...
export const deletePurchase = async (req, res) => {
    try {
        ...
        for (const item of purchase.products) {
            ...
            if (inv) {
                inv.storedStock = Math.max(0, inv.storedStock - item.quantity);
                inv.lastUpdated = new Date();
                await inv.save();
            }
        }
