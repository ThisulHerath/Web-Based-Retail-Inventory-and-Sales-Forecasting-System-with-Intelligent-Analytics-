...
const withInventory = async (product) => {
    ...
    const displayedStock = inv ? inv.displayedStock : 0;
    const storedStock = inv ? inv.storedStock : 0;
    const totalStock = displayedStock + storedStock;
    return {
        ...prodData,
        // Keep currentStock for compatibility; it now means displayed stock.
        currentStock: displayedStock,
        displayedStock,
        storedStock,
        totalStock,
        isLowStock: displayedStock <= product.minimumStockLevel,
    };
};
...
export const createProduct = async (req, res) => {
    try {
        ...
        await Inventory.create({ product: product._id, displayedStock: 0, storedStock: 0 });
...
export const updateProduct = async (req, res) => {
    try {
        ...
        const { productName, sku, category, description, costPrice, sellingPrice, minimumStockLevel, isActive, removeImage } = req.body;
        ...
        // Handle explicit image removal (without uploading a replacement).
        if (shouldRemoveImage && !req.file && product.productImage) {
            const oldImagePath = path.join(__dirname, '../public/uploads', product.productImage);
            if (fs.existsSync(oldImagePath)) {
                fs.unlinkSync(oldImagePath);
            }
            updates.productImage = null;
        }
