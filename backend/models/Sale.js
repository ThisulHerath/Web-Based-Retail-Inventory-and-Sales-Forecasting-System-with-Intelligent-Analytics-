...
const Sale = {
    ...
    async getAnalyticsData() {
        const { data: salesData, error: salesError } = await supabase
            .from(TABLE)
            .select('payment_method, grand_total, total_profit, created_at');
    ...
    async updateById(id, updates) {
        ...
        if (updates.discountAmount !== undefined) row.discount_amount = updates.discountAmount;
        if (updates.discountedSubtotal !== undefined) row.discounted_subtotal = updates.discountedSubtotal;
    ...
    format(row, items = []) {
        ...
        const formattedItems = items.map(i => ({
            ...
            quantity: i.quantity,
            costPrice: toNumber(i.cost_price),
        ...
        return {
            ...
            discountedSubtotal: row.discounted_subtotal !== null && row.discounted_subtotal !== undefined
                ? toNumber(row.discounted_subtotal)
                : toNumber(row.subtotal) - toNumber(row.discount_amount),
