...
const Inventory = {
    ...
    async create(obj) {
        const displayedStock = obj.displayedStock !== undefined
            ? obj.displayedStock
            : (obj.currentStock || 0);
        const row = {
            product_id: obj.product,
            current_stock: displayedStock,
            displayed_stock: displayedStock,
            stored_stock: obj.storedStock || 0,
    ...
    async updateById(id, updates) {
        const row = {};
        const nextDisplayed = updates.displayedStock !== undefined
            ? updates.displayedStock
            : updates.currentStock;
        if (nextDisplayed !== undefined) {
            row.displayed_stock = nextDisplayed;
            // Keep legacy column aligned for compatibility with older reads.
            row.current_stock = nextDisplayed;
        }
        if (updates.storedStock !== undefined) row.stored_stock = updates.storedStock;
    ...
    async aggregate(pipeline) {
        ...
        (invData || []).forEach(inv => {
            ...
            const displayed = inv.displayed_stock !== undefined && inv.displayed_stock !== null
                ? inv.displayed_stock
                : (inv.current_stock || 0);
            const stored = inv.stored_stock || 0;
            totalValue += (displayed + stored) * costPrice;
        });
    ...
    format(row) {
        if (!row) return null;
        const displayedStock = row.displayed_stock !== undefined && row.displayed_stock !== null
            ? row.displayed_stock
            : (row.current_stock || 0);
        const storedStock = row.stored_stock || 0;
        const totalStock = displayedStock + storedStock;
        return {
            _id: row.id,
            id: row.id,
            product: row.product_id,
            productId: row.product_id,
            // Keep currentStock for compatibility; it now maps to displayed stock.
            currentStock: displayedStock,
            displayedStock,
            storedStock,
            totalStock,
            ...
            async save() {
                return await Inventory.updateById(row.id, {
                    displayedStock: this.displayedStock !== undefined ? this.displayedStock : this.currentStock,
                    storedStock: this.storedStock,
