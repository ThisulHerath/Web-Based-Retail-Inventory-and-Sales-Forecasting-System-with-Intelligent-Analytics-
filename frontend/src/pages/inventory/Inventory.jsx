                ...
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
                    <div className="relative sm:col-span-1">
                        ...
                        <input value={search} onChange={(e) => { setSearch(e.target.value); setCurrentPage(1); }}
                            ...
                            className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#f5d800] outline-none text-sm" />
                    </div>
                    <div className="relative">
                        <Filter className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                        <select value={categoryFilter} onChange={(e) => { setCategoryFilter(e.target.value); setCurrentPage(1); }}
                            className="w-full pl-9 pr-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#f5d800] outline-none text-sm appearance-none">
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="border-b border-gray-200">
                                        ...
                                        <th className="text-center py-3 px-4 font-semibold text-gray-700 text-sm">Displayed</th>
                                        <th className="text-center py-3 px-4 font-semibold text-gray-700 text-sm">Stored</th>
                                        <th className="text-center py-3 px-4 font-semibold text-gray-700 text-sm">Total</th>
                                ...
                                <tbody>
                                    {inventory.map((inv) => {
                                        const displayedStock = inv.displayedStock ?? inv.currentStock ?? 0;
                                        const storedStock = inv.storedStock ?? 0;
                                        const totalStock = inv.totalStock ?? (displayedStock + storedStock);
                                        const isLow = displayedStock <= inv.product?.minimumStockLevel;
                                        return (
                                            <tr key={inv._id} className="border-b border-gray-100 hover:bg-gray-50 transition-colors">
                                                ...
                                                <td className="py-3 px-4 text-center"><span className="text-lg font-bold text-indigo-700">{storedStock}</span></td>
                                                <td className="py-3 px-4 text-center"><span className="text-lg font-bold text-gray-900">{totalStock}</span></td>
