...
const EditSale = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (invalidItems.length > 0) {
            const hasMissingProduct = invalidItems.some(item => !item.productId);
            const outOfStock = invalidItems.some(item => item.productId && item.quantity > item.maxStock);
            setToast({
                message: hasMissingProduct
                    ...
                    <div className="bg-white rounded-xl shadow-md p-6 border border-gray-100">
                        ...
                        <div className="space-y-4">
                            {formData.items.map((item, index) => (
                                <div
                                    key={index}
                                    className="grid grid-cols-1 md:grid-cols-6 gap-4 p-4 bg-gray-50 rounded-lg items-start"
                                >
                                    ...
                                    <div className="flex justify-center pt-8">
                                        {formData.items.length > 1 && (
                                            <button
                                                ...
                                                className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50 border border-transparent hover:border-red-200 rounded-lg transition-colors mt-1"
                                            >
